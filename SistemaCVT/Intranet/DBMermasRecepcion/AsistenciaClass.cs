...
namespace DBMermasRecepcion
{
    public class AsistenciaClass
    {
        ...
        public bool InsertaRegistroAsistencia(CVT_Asistencia nuevo)
        {
            bool ret = false;
            try
            {
                DBDesaint.CommandTimeout = 6000;
                DBDesaint.CVT_Asistencia.InsertOnSubmit(nuevo);
        ...
        public DataTable SP_COMPARATIVOAREALABORASITENCIA(DateTime fini, DateTime fter)
        {
            DataTable ret = new DataTable();
            try
            {
                ret = Utilidades.LINQToDataTable(DBDesaint.SP_COMPARATIVO_AREA_LABOR_ASITENCIA(fini, fter));
            }
            catch (Exception)
            {
        ...
        public DataTable SP_COMPARATIVOAREAPERSONALABORASITENCIA(DateTime fini, DateTime fter)
        {
            DataTable ret = new DataTable();
            try
            {
                ret = Utilidades.LINQToDataTable(DBDesaint.SP_COMPARATIVO_DETALLE_ASITENCIA(fini, fter));
            }
            catch (Exception)
            {
