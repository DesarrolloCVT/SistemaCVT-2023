...
{
    public partial class ListadoPlantillasRegistros : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                string ruta = @"\\BIGEL\PLANTILLAS DE REGISTRO";
                CargarGridviewConArchivos(ruta);
                LogClass vLog = new LogClass();
                vLog.LOGUsabilidad(Convert.ToInt32(Session["IDCVTUsuario"]), 95, "Ingreso");
            }
            //else
            //{
            //    try
        ...
        private void CargarGridviewConArchivos(string ruta)
        {
            ...
            if (ruta == @"\\BIGEL\PLANTILLAS DE REGISTRO")
            {
        ...
        protected void GvDatos_RowCommand(object sender, DevExpress.Web.ASPxGridViewRowCommandEventArgs e)
        {
            ...
            {
                LogClass vLog = new LogClass();
                vLog.LOGUsabilidad(Convert.ToInt32(Session["IDCVTUsuario"]), 95, "Boton");
