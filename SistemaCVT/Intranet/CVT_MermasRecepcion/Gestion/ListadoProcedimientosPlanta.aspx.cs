...
{
    public partial class ListadoProcedimientosPlanta : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                string ruta = @"\\BIGEL\PROCEDIMIENTOS CALIDAD";
                CargarGridviewConArchivos(ruta);
                LogClass vLog = new LogClass();
                vLog.LOGUsabilidad(Convert.ToInt32(Session["IDCVTUsuario"]), 93, "Ingreso");
            }
            //if(Session["Ruta"]!=null)
            //{
            //    CargarGridviewConArchivos(Session["Ruta"].ToString());
            //}
        }
        private void CargarGridviewConArchivos(string ruta)
        {
            ...
            if (ruta == @"\\BIGEL\PROCEDIMIENTOS CALIDAD")
            {
        ...
        protected void GvDatos_RowCommand(object sender, DevExpress.Web.ASPxGridViewRowCommandEventArgs e)
        {
            ...
            {
                LogClass vLog = new LogClass();
                vLog.LOGUsabilidad(Convert.ToInt32(Session["IDCVTUsuario"]), 93, "Boton");
                ...
                if (Tipo == "C")
                {
                    ...
                    Session["Ruta"] = e.KeyValue.ToString();
                }
            }
        }
